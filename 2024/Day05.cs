...
public static class Day05
{
    public static void Run()
    {
        ...
        void Execute(string updateInstructions)
        {
            var parts = updateInstructions.Split(Environment.NewLine + Environment.NewLine);
            var orderingRules = parts[0].Split(Environment.NewLine).Select(l => l.Split("|").Select(int.Parse).ToArray()).ToArray();
            var pagesToUpdate = parts[1].Split(Environment.NewLine).Select(l => l.Split(",").Select(int.Parse).ToList());

            var part1 = 0;

            foreach (var pageUpdates in pagesToUpdate)
            {
                var validPageUpdates = orderingRules.All(rule =>
                {
                    var firstIndex = pageUpdates.FindIndex(p => p == rule[0]);
                    var secondIndex = pageUpdates.FindIndex(p => p == rule[1]);
                ...
                if (validPageUpdates)
                {
                    part1 += pageUpdates[pageUpdates.Count / 2];
                }
            }
            ...
            foreach (var pageUpdates in pagesToUpdate)
            {
                ...
                while (true)
                {
                    ...
                    var invalidRule = orderingRules.FirstOrDefault(rule =>
                    {
                        firstIndex = pageUpdates.FindIndex(p => p == rule[0]);
                        secondIndex = pageUpdates.FindIndex(p => p == rule[1]);
                    ...
                    if (invalidRule is null)
                    {
                        if (fixedPage)
                        {
                            part2 += pageUpdates[pageUpdates.Count / 2];
                        }
                        break;
                    }

                    (pageUpdates[firstIndex], pageUpdates[secondIndex]) = (pageUpdates[secondIndex], pageUpdates[firstIndex]);
                    fixedPage = true;
                }
            }
